...
rules = [
    {
        # 规则 1: 如果库存低于 20
        "conditions": {"all": [{"name": "current_inventory", "operator": "less_than", "value": 20}]},
    ...
    },
    {
        ...
        "conditions": {"all": [{"name": "days_to_expire", "operator": "less_than", "value": 10}]},
        "actions": [{"name": "log_action", "params": {"message": "Product is about to expire!"}}],
    },
    {
        # 规则 4: 品牌是否是 Brand A
        "conditions": {"all": [{"name": "brand", "operator": "equal_to", "value": "Brand A"}]},
        "actions": [{"name": "log_action", "params": {"message": "Product is of brand A!"}}],
    },
    {
        # 规则 5: 比较库存差异是否大于 30
        "conditions": {"all": [{"name": "current_inventory_diff", "operator": "greater_than", "value": 30}]},
        "actions": [{"name": "log_action", "params": {"message": "Significant inventory difference detected!"}}],
    },
    {
        # 规则 6: 测试正则匹配缓存的功能
        "conditions": {"all": [{"name": "name", "operator": "matches_cached_regex", "value": "^Special.*"}]},
            ...
            "all": [
                {
                    "name": "addresses",
                    "operator": "contains",
                    "value": {"street": "456 Elm St", "city": "New York", "state": "NY", "zip_code": "54321"},
                }
            ]
        },
    ...
    {
        # 规则 6: 测试正则匹配缓存的功能
        "conditions": {"all": [{"name": "main_address_city", "operator": "equal_to", "value": "New York"}]},
        "actions": [{"name": "log_action", "params": {"message": "Main addrss.city equals_to New York!"}}],
    },
    {
        "conditions": {"all": [{"name": "addresses_city", "operator": "contains", "value": "New York"}]},
        "actions": [{"name": "log_action", "params": {"message": "Product city contains addresses in New York!"}}],
    },
    {
        "conditions": {"all": [{"name": "addresses_city", "operator": "contains_not", "value": "New York"}]},
